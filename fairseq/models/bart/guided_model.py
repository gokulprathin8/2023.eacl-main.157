...
class GuidedBARTModel(GuidedTransformerModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        #random-init only first decoder layer
#        self.decoder.reinit_first_layer()
        if args.loc_position == 0:
            self.decoder.append_takasedecoder_new_layer()
        elif args.loc_position == 2:
            self.decoder.append_decoder_new_layer()
        ...
        # mapping doc_embed to predict target length
        self.doc2tgt_maps=BARTClassificationHead(
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            '--num-topic', type=int, default=16,
            help='number of topic tokens'
        )
...
#        control_number=200
        doc_embeddings=encoder_out[-1]
        x_tgt=self.doc2tgt_maps(doc_embeddings) * 10 #loss divided by 10
#        x_tgt=torch.tensor(control_number, dtype=float, device=x_tgt.device)
#        x_tgt=x_tgt.repeat(4).unsqueeze(1)

        decoder_out = self.decoder(prev_output_tokens, encoder_out, incremental_state=incremental_state, test_tgt_len=x_tgt, **extra_args)
        return decoder_out
...
#            False,
        ...
        # mapping2one dim to predict tgt lengths
        doc_embeddings = encoder_out[-1]

        x_tgt= self.doc2tgt_maps(doc_embeddings)
